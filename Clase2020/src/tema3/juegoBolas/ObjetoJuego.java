...
public abstract class ObjetoJuego {
	...
	private static void calcCoords( ObjetoJuego o ) {
		System.out.println( o.getX() + o.getY() );
	}
	private static void dibujaTodo( ObjetoJuego[] objetos, tema3.VentanaGrafica v ) {
		// for (int i=0; i<objetos.length; i++) {
		for (ObjetoJuego oj : objetos) {
			// ObjetoJuego oj = objetos[i];
			oj.dibuja( v );
		}
	}
